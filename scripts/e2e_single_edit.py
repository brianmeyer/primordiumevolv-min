...
Examples:
  ...
  python scripts/e2e_single_edit.py --path app/config.py --match "DEBUG = False\\nLOG_LEVEL = \\"INFO\\"" --replace "DEBUG = True\\nLOG_LEVEL = \\"DEBUG\\"" --goal debug_mode --model gpt-4 --run-id DEBUG01
    ...
    parser.add_argument("--goal", required=True,
                       help="Goal tag for commit message and analytics")
    parser.add_argument("--model", required=True,
                       help="Model name for commit attribution and analytics")
    parser.add_argument("--run-id", required=True,
                       help="Unique run identifier for analytics")
    parser.add_argument("--area", default="test",
                       help="Area for the change (default: test)")
    parser.add_argument("--rationale", default="end-to-end test change",
    ...
    try:
        ...
        if result.get("ok"):
            ...
            print(f"✓ Modified {len(result.get('touched', []))} files: {result.get('touched', [])}")

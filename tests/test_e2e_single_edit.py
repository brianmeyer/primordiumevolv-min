...
class TestE2ESingleEdit(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Copy patcher module
        shutil.copy2(original_repo / 'app' / 'dgm' / 'patcher.py', dgm_dir / 'patcher.py')
...
'''
        ...
        scripts_dir.mkdir()
        shutil.copy2(original_repo / 'scripts' / 'e2e_single_edit.py', scripts_dir / 'e2e_single_edit.py')
    ...
    def test_basic_single_edit(self):
        ...
        result = subprocess.run([
            sys.executable, 'scripts/e2e_single_edit.py',
            '--path', self.test_file,
            '--match', 'X = 1',
            '--replace', 'X = 10',
            '--goal', 'test_basic',
    ...
    def test_custom_area_and_rationale(self):
        ...
        result = subprocess.run([
            ...
            '--run-id', 'TEST005',
            '--area', 'custom_area',
            '--rationale', 'custom test rationale'
        ], capture_output=True, text=True)
    ...
    def test_from_scripts_directory(self):
        ...
        result = subprocess.run([
            sys.executable, 'e2e_single_edit.py',
            '--path', f'../{self.test_file}',
            '--match', 'Y = 2',
            '--replace', 'Y = 200',
            '--goal', 'test_from_scripts',
            '--model', 'test_model',
            '--run-id', 'TEST006'
        ], capture_output=True, text=True)
    ...
    def test_help_message(self):
        ...
        self.assertIn('E2E Single Edit Test', result.stdout)

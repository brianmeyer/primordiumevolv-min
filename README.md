...
- Meta‑Evolution: UCB1 bandit with advanced total_reward system (outcome + process - cost) featuring two-judge AI evaluation with tie-breaker over operators (system, nudge, temp, memory, RAG, web). All generation uses local Ollama.
...
- Long generation: there is no app‑side token cap; generation length is controlled by the local Ollama model. Consider using a smaller/faster local model for quicker iteration.
